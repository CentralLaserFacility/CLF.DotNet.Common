...
namespace Clf.Common.ImageProcessing
{
  ...
  //
  // This class owns an array-of-bytes that represents the intensities
  ...
  //    +---+---+---+
  //
  // Physical representation as a 1-D array of bytes :
  //
  //    +---+---+---+---+---+---+
  //    | 0 | 1 | 2 | 3 | 4 | 5 |  
  //    +---+---+---+---+---+---+
  ...
  //
  // In a future variant we'll support 12-bit and 16-bit intensity values.
  //

  public partial class LinearArrayHoldingPixelBytes
  {

    public static LinearArrayHoldingPixelBytes Create_WithRandomValues (
      int width,
      int height
    ) {
      return new LinearArrayHoldingPixelBytes(
        width,  
        height,
        (x,y) => (byte) System.Random.Shared.Next()
      ) ;
    }
    ...
    public static LinearArrayHoldingPixelBytes Create_WithRandomValues (
      ...
      var patchValues = new byte[nPatchesX+1,nPatchesY+1] ;
      for ( int y = 0 ; y <= nPatchesY ; y++ )
      {
        for ( int x = 0 ; x <= nPatchesX ; x++ )
        {
          patchValues[x,y] = (byte) System.Random.Shared.Next() ;
        }
      }
      return new LinearArrayHoldingPixelBytes(
        width,  
    ...
    public IReadOnlyList<byte> LinearArrayValues => m_linearArray ;

    public int InstanceNumber {  get ; }

    internal byte[] m_linearArray ; // This is directly accessed via an unsafe pointer ...

    private static int m_nInstancesCreated = 0 ;

    // Create an instance, allocating a new byte array of appropriate size,
    // with all the pixel values initialised to zero.

    public LinearArrayHoldingPixelBytes (
      int                        width,
      int                        height
    ) {
      ...
      Width  = width ;
      Height = height ;
      m_linearArray = new byte[PixelCount] ;
    }
    ...
    public LinearArrayHoldingPixelBytes (
      int                       width,
      int                       height,
      System.Func<int,int,byte> evaluatePixelFunc
    ) :
    this(
      width,
      height
    ...
    public void LoadLinearArrayWithAscendingValues_ForTesting ( )
    {
      ...
      {
        m_linearArray[i] = (byte) i ;
      }
    }

    // Create an instance, taking ownership of the byte-array passed in.
    ...
    public LinearArrayHoldingPixelBytes (
      int    width,
      int    height,
      byte[] linearArrayThatWeWillTakeOwnershipOf
    ) {
      InstanceNumber = m_nInstancesCreated++ ;
      Width  = width ;
      Height = height ;
    ...
    public byte MaxValue => m_linearArray.Max() ;

    public byte MinValue => m_linearArray.Min() ;
    ...
    public IReadOnlyList<byte> GetRowOfPixelsAtOffsetFromTop ( int y ) 
    {
    ...
    public IReadOnlyList<byte> GetColumnOfPixelsAtOffsetFromLeft ( int x ) 
    {
    ...
    public void SetAllPixelsFromSource ( IEnumerable<byte> source )
    {
    ...
    public void SetAllPixelsInPlace ( System.Func<int,int,byte> getPixelValueFunc ) 
    {
      for ( int i = 0 ; i < m_linearArray.Length ; i++ ) 
      {
    ...
    public void SetAllPixelsInPlace ( byte pixelValue ) 
    {
      for ( int i = 0 ; i < m_linearArray.Length ; i++ ) 
      {
    ...
    public void SetAllPixelsInPlace ( IReadOnlyList<byte> pixelValues ) 
    {
      for ( int i = 0 ; i < m_linearArray.Length ; i++ ) 
      {
    ...
    public void ApplyNormalisationInPlace ( byte pixelValueToUseAsReference ) 
    {
    ...
    public void ApplyTransformationInPlace ( System.Func<byte,byte> transformationFunc ) 
    {
      for ( int i = 0 ; i < m_linearArray.Length ; i++ ) 
      {
    ...
    public LinearArrayHoldingPixelBytes CreateClone ( ) 
    => new LinearArrayHoldingPixelBytes(
      Width,
      Height,
      Helpers.CreateCloneOfByteArray(m_linearArray)
    ) ;

    public LinearArrayHoldingPixelBytes CreateTransformedClone (
      System.Func<byte,byte> transformationFunc
    ) {
      byte[] transformedValues = new byte[m_linearArray.Length] ;
      for ( int i = 0 ; i < m_linearArray.Length ; i++ ) 
      {
    ...
    public LinearArrayHoldingPixelBytes CreateNormalisedClone ( byte pixelValueToUseAsReference )
    {
    ...
    private static void NormaliseToReferenceValue ( 
      byte[] pixelArray, 
      byte   pixelValueToUseAsReference
    ) {
    ...
    public IReadOnlyList<int> BuildHistogram ( )
    {
      ...
      {
        byte pixelValue = m_linearArray[i] ;
        histogram[pixelValue]++ ;
      }
      return histogram ;
    }

    public LinearArrayHoldingPixelBytes CreateResizedClone_NearestNeighbour ( 
      int resizedWidthX, 
      int resizedHeightY
    ) {
      if ( 
      ...
      { 
        ...
        var resizedPixelArray = new LinearArrayHoldingPixelBytes(
          resizedWidthX,
          resizedHeightY
        ) ;
        ...
        for ( int yResized = 0 ; yResized < resizedHeightY ; yResized++ )
        {
          ...
          for ( int xResized = 0 ; xResized < resizedWidthX ; xResized++ )
          {
            ...
            byte originalPixel = this.m_linearArray[
              originalIndexX 
    ...
    public LinearArrayHoldingPixelBytes CreateRotatedClone ( RotationFactor rotationFactor )
    {
      return (
      ...
      LinearArrayHoldingPixelBytes ComputeRotatedClone ( )
      {
        return new LinearArrayHoldingPixelBytes(
          ComputeRotatedWidth(),
